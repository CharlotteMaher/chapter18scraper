...
#this scraper is for only five of the schools due to time constraints- you can add more ID codes- follow link in book to do this. 
...
def scrape_table(root):
    ...
    #there's more than one table, so we need to specify the class="destinations", represented by the full stop- dot shows class #is ID
    rows = root.cssselect("table.destinations tr")
    for row in rows:
        #Create a new empty record
        record = {}
        #Assign the contents of <td to a new object (variable) called 'table_cells'
        table_cells = row.cssselect("td")
        #If there's anything
        if table_cells: 
            #Put the contents of the first <td into a record in the column 'FSM' for free school meals
...
def scrape_page(url):
    #now 'url' is scraped with the scraperwiki library imported above, and the contents put into a new object (variable), 'html'
    html = scraperwiki.scrape(url)
    print html
    #now we use the lxml.html function imported above to convert 'html' into a new object (variable), 'root'
...
schoolIDs =['5237521','5244439','5237629','5237823','5234026'] #these are a list due to [']
#go through the schoolIDs list above, and for each ID...
for item in schoolIDs:
    #item is the name of the variable created when the for loop iterates through the list of school ID codes: 
    #in other words it is the ID code of the particular page that’s being scraped.
